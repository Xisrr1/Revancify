...
configure() {
    ...
    if [ "$PREV_BETA" = "off" ] && [ "$REVANCIFY_XISR_BETA" = "on" ]; then
        if ! "${DIALOG[@]}" --title "Enable Beta Updates?" --yes-label "Confirm" --no-label "Cancel" --yesno "Are you sure you want to enable Revancify 𝕏isr beta updates?\n\nThis option enables beta/pre-release updates for Revancify 𝕏isr itself.\nIt does NOT affect the stability or update channel of patches or the apps you install.\n\nBeta versions may include new features and bug fixes, but could also be less stable than regular releases.\n\nIf Revancify 𝕏isr fails to launch after enabling beta, you can recover by running 'revancify -f'.\n\nYou can always turn this option off in the Configure menu.\n\nProceeding will restart Revancify 𝕏isr." -1 -1; then
